...
\begin{abstract}
Our objective for the team project is to create a comprehensive survey paper over the usage of graph neural networks and deep learning in the domain of programming languages. The paper will be titled “GNN In Programming Language”. Till now we have seen several advancements in programming techniques which use graph neural networks to comprehend programming languages. We have also seen works being done in bug detection, similarity checking, synthesis, semantics, decompilation and translation of programs. There is also a growing interest in combining programming languages with other domains like NLP (Code x NLP). These subdomains however are in their infancy and have huge potentials for advancements. In the current state these subdomains are separate and interdomain work is limited. This leads to lack of survey papers which connect these domains and show the complete potential of using graph neural networks in the complete process of programming. This survey paper aims to diminish the separation between these subdomains by aggregating the state of the art work in these subdomains and providing background information to understand the current and potential usage of graph neural networks in programming languages. In this paper we will be focusing on the aforementioned subdomains and going in detail over the state of the art techniques in each one of them. We will also start with an overview of how to represent code using Graph Neural Networks, since this will be the parent step in most applications.
For this project we will focus on collecting a comprehensive text on related works and background information for the first week. For the next two and a half weeks we will analyze the collected text and implement the models proposed (using the git repositories provided or following the implementation section). Finally we will accrue our analysis into a comprehensive paper along with code used for evaluation in the final one to one and half weeks.
\end{abstract}
